...
Each installation owner must first register a Google Desktop OAuth client and/or Microsoft Entra public client, then configure the app identity through the clearly labeled **Administrator setup** area or deployment environment variables. The local desktop build persists that installation-level identity in the Git-ignored `runtime/oauth_clients.json`; the public repository remains blank. Provider-hosted consent is the only mailbox sign-in path. Access and refresh tokens stay in server memory, never return to the browser, and clear when the process stops. Do not paste mailbox passwords, client secrets, access tokens, or refresh tokens into chat, policy JSON, source events, logs, reports, or Git. Production should inject app identities and store user tokens in an approved secret manager. See the exact provider setup steps in [Company quick start](company_quickstart.md).
