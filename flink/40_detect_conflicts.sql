...
SELECT
  ...
  CASE
    WHEN a.entity_id IS NULL OR r.supporting_event_id IS NULL THEN 'INSUFFICIENT_EVIDENCE'
  ...
  CASE
    WHEN a.entity_id IS NULL OR r.supporting_event_id IS NULL THEN 'REVIEW'
  ...
  END AS decision,
  CASE
    WHEN a.entity_id IS NULL OR r.supporting_event_id IS NULL THEN 'HIGH'
  ...
  ARRAY[a.event_id, r.supporting_event_id] AS evidence_event_ids,
  CASE
    WHEN a.entity_id IS NULL OR r.supporting_event_id IS NULL
  ...
  END AS explanation,
  CASE
    WHEN a.entity_id IS NULL OR r.supporting_event_id IS NULL THEN ARRAY['CG-005-REQUIRED-PROVENANCE']
  ...
  CASE
    WHEN a.entity_id IS NULL OR r.supporting_event_id IS NULL THEN TRUE
  ...
  CASE
    WHEN a.entity_id IS NULL OR r.supporting_event_id IS NULL THEN 'PENDING'
  ...
  JSON_OBJECT(
    ...
    'supporting_event_id' VALUE r.supporting_event_id,
